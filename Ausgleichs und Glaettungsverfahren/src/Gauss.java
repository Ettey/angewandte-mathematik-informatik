...
public class Gauss {
	public static void main(String[] args) {
		double[][] matrix = {{1.0, 8.0, 0.0},
				{1.0,  0.0, -8.0},
				{1.0, 0.0, -8.0}};
		double[] vector = {2,0,0};

		/*
		 * "true" oder "false" kann als dritter Parameter uebergeben werden. "true" ->
		 * Zwischenschritte werden auf der Konsole ausgegeben "false" -> nichts wird
		 * ausgegeben
		 */
	...
	public static double[] getSolution(double[][] matrix, double[] vector, boolean printSteps) {
		...
		for (int line = 0; line < matrix.length; line++) {
			...
			for (int row = line + 1; row < matrix.length; row++) {
				// Wenn die Zwischenschritte ausgegeben werden sollen
				if (printSteps) {
					System.out.println("Zu Zeile " + (row + 1) + " wird subtrahiert: " + matrix[row][tmpColumn]
							+ " * Zeile " + (line + 1));
				}
		...
		for (int column = matrix[0].length - 1; column > 0; column--) {
			...
			for (int row = column; row > 0; row--) {
				// Wenn die Zwischenschritte ausgegeben werden sollen
				if (printSteps) {
					System.out.println("Zu Zeile " + (row) + " wird subtrahiert: " + matrix[row - 1][column]
							+ " * Zeile " + (column + 1));
				}
	...
	/*
	 * Eine Zeile (row) wird mit einem entsprechendem vielfachen (factor) von einer
	 * anderen Zeile (rowRoot) subtrahiert.
	 */
	private static void removeRowLeadingNumber(double factor, int rowRoot, int row, double[][] matrix,
			double[] vector) {
		for (int column = 0; column < matrix[row].length; column++) {
